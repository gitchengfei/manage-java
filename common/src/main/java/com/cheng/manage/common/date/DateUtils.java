package com.cheng.manage.common.date;
...
public class DateUtils {
    ...
    public static Date dateAdd(Date date, int days, boolean includeTime) {
        try {
            ...
            e.printStackTrace();
            throw new MyException(ResultEnum.DATE_TRANSFORM_ERROR);
        }
    }
    ...
    /**
     * 作者: cheng fei
     * 创建日期: 2019/3/19 22:47
     * 描述 : 获取下个月的总天数
     * @param date
     * @return
     */
    public static int getNextMonthDays(Date date) throws ParseException {
        ...
        if (month == 12){
            nextMonthFirstDay = (year + 1) + "-01-01";
        } else {
            nextMonthFirstDay = year + "-" + (month + 1) + "-01";
        }
        Date nextMonth = dateParse(nextMonthFirstDay, DateUtils.DATE_PATTERN);

        return getDaysOfMonth(nextMonth);
    }

    /**
     * 作者: cheng fei
     * 创建日期: 2019/3/19 23:24
     * 描述 : 获取下个月第一天日期
     * @param date
     * @return
     * @throws ParseException
    ...
    public static Date getNextMonthFirstDay(Date date) throws ParseException {
        ...
        return dateParse(nextMonthFirstDay, DATE_PATTERN);
    }
     ...
     * 作者: cheng fei
     * 创建日期: 2019/3/19 22:47
     * 描述 : 获取下个月第几天的日期,如超过最大天数返回null
     *        例如:2月第30天,返回null
     * @param date
     * @param day
     * @return
     */
    public static Date getNextMonthDate(Date date, int day) throws ParseException {
        int year = getYear(date);
        int month = getMonth(date);
        ...
        Date nextMonth = dateParse(nextMonthFirstDay, DATE_PATTERN);
        int countNextMonthDay = getDaysOfMonth(nextMonth);
        if (countNextMonthDay >= day){
            String nextMonthDay = nextMonthFirstDay.substring(0, nextMonthFirstDay.lastIndexOf("-") + 1) + (day < 10 ? "0" + day : day);
            return dateParse(nextMonthDay, DATE_PATTERN) ;
        }
        return null;
    }
    ...
    /**
     * 作者: cheng fei
     * 创建日期: 2019/3/19 23:04
     * 描述 : 获取当月的第几天日,如果超出当月最大天数,返回null
     *        例如:2月第30天,返回null
     * @param date
     * @param day
     * @return
     * @throws ParseException
     */
    public static Date getNowMonthDate(Date date, int day) throws ParseException {
        int year = getYear(date);
        int month = getMonth(date);

        //当月一号时间
        String oneDay = year + "-" + month  + "-01";
        Date nowMonthFirstDay = dateParse(oneDay, DATE_PATTERN);
        int countNowMonthDay = getDaysOfMonth(nowMonthFirstDay);
        if (countNowMonthDay >= day){
            String nextMonthDay = oneDay.substring(0, oneDay.lastIndexOf("-") + 1) + (day < 10 ? "0" + day : day);
            return dateParse(nextMonthDay, DATE_PATTERN) ;
        }
        return null;
    }
...
//        System.out.println(getDate(dateParse("2017-01-17", null)))
        System.out.println(dateAddMonths(dateParse("2019-01-30", null), 1));
        /*
